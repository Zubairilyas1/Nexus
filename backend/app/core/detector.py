...
def create_detector(
    ...
    """Factory function to create detector with backend auto-detection.

    Args:
        model_path: Path to ONNX model
        backend: "auto", "tensorrt", "cuda", or "cpu"
    """
    if backend == "tensorrt":
        try:
            from app.core.tensorrt_detector import TensorRTDetector
            trt_path = model_path.replace(".onnx", ".plan")
            if Path(trt_path).exists():
                return TensorRTDetector(engine_path=trt_path)
        except ImportError:
            pass
        # Fallback to ONNX
